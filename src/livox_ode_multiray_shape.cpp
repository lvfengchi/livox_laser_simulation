...
//////////////////////////////////////////////////
LivoxOdeMultiRayShape::LivoxOdeMultiRayShape(CollisionPtr _parent) : MultiRayShape(_parent) {
    ...
    ODELinkPtr pLink = boost::static_pointer_cast<ODELink>(this->collisionParent->GetLink());
...
void LivoxOdeMultiRayShape::UpdateRays() {
    ...
    // especially when spawning models with sensors
    {
        ...
        dSpaceCollide2((dGeomID)(this->superSpaceId), (dGeomID)(ode->GetSpaceId()), this, &UpdateCallback);
    }
}
...
void LivoxOdeMultiRayShape::UpdateCallback(void *_data, dGeomID _o1, dGeomID _o2) {
    ...
    if (dGeomIsSpace(_o1) || dGeomIsSpace(_o2)) {
        if (dGeomGetSpace(_o1) == self->superSpaceId || dGeomGetSpace(_o2) == self->superSpaceId)
            dSpaceCollide2(_o1, _o2, self, &UpdateCallback);

        if (dGeomGetSpace(_o1) == self->raySpaceId || dGeomGetSpace(_o2) == self->raySpaceId)
            dSpaceCollide2(_o1, _o2, self, &UpdateCallback);
    } else {
...
double gazebo::physics::LivoxOdeMultiRayShape::GetRange(unsigned int _index) {
    if (_index >= this->rays.size()) {
        std::ostringstream stream;
        stream << "index[" << _index << "] out of range[0-" << this->rays.size() << "]";
        gzthrow(stream.str());
    }
...
void gazebo::physics::LivoxOdeMultiRayShape::Update() {
    // The measurable range is (max-min)
    static double fullRange = this->GetMaxRange() - this->GetMinRange();
